...
export const PopulateScheduleEmptyAlertDialog: FunctionComponent<PopulateScheduleEmptyAlertDialogProps> = ({
  schedule,
  scheduler,
}) => {
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger
        disabled={isDisabled()}
        className="hover:bg-accent rounded px-2 py-1 text-start text-sm"
      >
        Empty Zonebooks
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Populate this month&apos;s schedule with meter readers only?</AlertDialogTitle>
          <AlertDialogDescription>
            This will populate all meter readers with their respective rest days and empty zonebooks for this
            month.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
