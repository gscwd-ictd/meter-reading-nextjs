...
export const useScheduler = (holidays: Holiday[]) => {
  ...
  //   // dueDate = addDays(readingDate, 15); //! revert this back
  ...
  //   while (isSameMonth(readingDate, monthStart) && readingCount < 22) {
  //     // Calculate due date for the current reading date
  //     dueDate = addDays(readingDate, 15);
  //     dueDate = adjustForHolidayOrWeekend(dueDate);

  //     // Push the current reading date with its due date
  //     dueDates.push({ readingDate, dueDate });

  //     // Increment reading date for next loop
  //     readingDate = addDays(readingDate, 1);
  //     readingCount++;
  //   }
  ...
  //   while (isSameMonth(readingDate, monthStart) && readingCount < 22) {
  //     // 1. Calculate due date from the current reading date (not from previous dueDate)
  //     let dueDate = addDays(readingDate, 15);

  //     // 2. Adjust if due date falls on weekend, holiday, or no-duty day
  //     dueDate = adjustForHolidayOrWeekend(dueDate);

  //     // 3. Push current reading date with its due date
  //     dueDates.push({ readingDate, dueDate });

  //     // 4. Increment reading date by 1 day
  //     readingDate = addDays(readingDate, 1);
  //     readingCount++;
  //   }
  ...
  //     if (!lastDueDate) {
  //       // First reading: calculate from reading date
  //       dueDate = addDays(readingDate, 15);
  //       dueDate = adjustForHolidayOrWeekend(dueDate);
  //     } else if (isSunday(readingDate)) {
  //       // Sunday shares Saturday's due date (lastDueDate)
  //       dueDate = lastDueDate;
  //     } else {
  //       // Increment from previous due date
  ...
  //     // Update lastDueDate only if it's not a Sunday
  //     if (!isSunday(readingDate)) {
  //       lastDueDate = dueDate;
  //     }
  ...
  //       dueDate = adjustForHolidayOrWeekend(dueDate);
  //     } else if (isSunday(readingDate)) {
  //       // Sunday shares Saturday's due date
  //       dueDate = lastDueDate;
  //     } else {
  //       // Increment from previous due date
  //       dueDate = addDays(lastDueDate, 1);
  ...
  //         // If Saturday, check if next day is Sunday and in the same month
  //         const nextDay = addDays(readingDate, 1);
  //         if (isSameMonth(nextDay, monthStart) && getDay(nextDay) === 0) {
  //           readingCount++; // count Sat+Sun as one
  //         } else {
  //           readingCount++; // normal Saturday (no Sunday after)
  //         }
  //       } else {
  //         readingCount++; // normal weekday
  //       }
  //     }

  //     // Update lastDueDate only if it's not Sunday
  //     if (!isSunday(readingDate)) {
  //       lastDueDate = dueDate;
  //     }
  ...
  const calculateDueDates = useCallback((): DueDate[] => {
    ...
    while (isSameMonth(readingDate, monthStart) && slotCount < 22) {
      ...
      // Determine slot counting logic
      if (!isSunday(readingDate)) {
        if (getDay(readingDate) === 6) {
          // It's Saturday
          const nextDay = addDays(readingDate, 1);

          // Check if next day is Sunday and in the same month
          if (isSameMonth(nextDay, monthStart) && getDay(nextDay) === 0) {
            // This is a Sat-Sun pair → count once
            slotCount++;

            // If this is the last allowed slot and Saturday has a Sunday pair,
            // we still include the Sunday (Option A)
            if (slotCount >= 22) {
              // Add the Sunday now before breaking
              dueDates.push({ readingDate: nextDay, dueDate }); // Sunday shares Saturday's due date
            }
          } else {
            // No Sunday after, count normally
            slotCount++;
          }
        } else {
          // Normal weekday
      ...
      // Stop after adding Sunday for last slot if needed
      if (slotCount >= 22) break;
